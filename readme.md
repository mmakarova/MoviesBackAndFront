...
1. При открытие на странице показываются редкатор ввода фильма, редактор выбора жанра, кнопка Поиск и фильмы для певой страницы (количество фильмов 4 на странице). Для заполнения фильмов для первой загрузки необходимо выполнить запрос: **/api/movies?pagesize=4&page=1**
...
4. При нажатие кнопки Поиск посылается запрос на backend c параметрами. Получение параметров:
